...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/galerias'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Galerias) {
        return item.id;
    }
    registerChangeInGalerias() {
        this.eventSubscriber = this.eventManager.subscribe('galeriasListModification', (response) => this.loadAll());
    }
